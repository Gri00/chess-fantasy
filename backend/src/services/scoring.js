...
export function calculatePlayerScore(performance, tournament, rules = DEFAULT_RULES) {
...
export async function processScoringPeriod(periodId) {
  ...
  if (periodError || !period) {
    throw new Error(`Period ${periodId} nije pronađen`)
  }

  if (period.is_finalized) {
    console.log(`[Scoring] Period ${periodId} je već finalizovan, skip.`)
    return
  }
  ...
  if (!members || members.length === 0) {
    console.log(`[Scoring] Nema članova u ligi ${period.league.id}`)
    return
  }
  ...
  if (!tournaments || tournaments.length === 0) {
    console.log(`[Scoring] Nema završenih turnira u periodu ${periodId}`)
    return
  }
  ...
  for (const member of members) {
    ...
    for (const perf of performances) {
      ...
      const { points, breakdown } = calculatePlayerScore(perf, tournament, rules)
  ...
  if (scoreInserts.length === 0) {
    console.log(`[Scoring] Nema skorova za upisati u period ${periodId}`)
  } else {
    ...
    if (insertError) {
      throw new Error(`Greška pri upisu scorova: ${insertError.message}`)
    }

    console.log(`[Scoring] Upisano ${scoreInserts.length} scorova za period ${periodId}`)
  }
  ...
  console.log(`[Scoring] Period ${periodId} finalizovan.`)
...
export async function createScoringPeriods(leagueId) {
  ...
  if (!league.season_start || !league.season_end) {
    throw new Error('Liga nema definisan season_start i season_end')
  }
  ...
  if (error) throw new Error(`Greška pri kreiranju perioda: ${error.message}`)

  console.log(`[Scoring] Kreirano ${periods.length} perioda za ligu ${leagueId}`)
  return data
}
...
export async function addTestPerformanceAndScore(leagueId, chessPlayerId) {
  ...
  console.log('[Scoring] Test performansa dodana:', perf)
