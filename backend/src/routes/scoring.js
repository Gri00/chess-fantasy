...
export default async function scoringRoutes(app) {
  ...
  app.post('/leagues/:id/activate', {
    ...
    if (league.commissioner_id !== request.user.id) {
      return reply.code(403).send({ error: 'Samo komisar može aktivirati ligu' })
    }

    if (league.status !== 'pending') {
      return reply.code(400).send({ error: 'Liga je već aktivna ili završena' })
    }

    if (!league.season_start || !league.season_end) {
      return reply.code(400).send({ error: 'Postavi season_start i season_end pre aktivacije' })
    }
  ...
  app.post('/leagues/:id/score/:period_id', {
    onRequest: [authenticate]
  }, async (request, reply) => {
    ...
    if (!league || league.commissioner_id !== request.user.id) {
      return reply.code(403).send({ error: 'Samo komisar može pokrenuti scoring' })
    }

    try {
      const result = await processScoringPeriod(period_id)
      return reply.send({ message: 'Scoring završen', ...result })
    } catch (err) {
      app.log.error(err)
  ...
  app.get('/leagues/:id/scores', {
    ...
    if (error) {
      app.log.error(error)
      return reply.code(500).send({ error: 'Greška pri dohvatanju scorova' })
    }
  ...
  app.get('/leagues/:id/periods', {
    ...
    if (error) {
      return reply.code(500).send({ error: 'Greška pri dohvatanju perioda' })
    }
  ...
  app.post('/leagues/:id/test-score', {
    onRequest: [authenticate]
  }, async (request, reply) => {
    const { id } = request.params
    ...
    try {
      ...
      return reply.send({ message: 'Test performansa dodana', ...result })
    } catch (err) {
      app.log.error(err)
      return reply.code(500).send({ error: err.message })
    }
