...
export default async function leagueRoutes(app) {
  ...
  app.post('/', {
    ...
    const {
      name, description, league_type = 'private',
      roster_size = 5, max_teams = 20,
      tier_limits, scoring_rules,
      picks_deadline, season_start, season_end,
      team_name
    } = request.body
    ...
    // Komisar automatski postaje član
    ...
    if (memberError) {
      ...
      // Rollback — obrisi ligu ako member insert nije uspeo
  ...
  app.get('/', {
    ...
    let query = supabaseAdmin
      .from('leagues')
      .select(`
        id, name, description, league_type, status,
        roster_size, max_teams, season_start, season_end,
        created_at, invite_code,
        commissioner:users!commissioner_id(username, avatar_url),
  ...
  app.get('/mine', {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select(`
        ...
        league:leagues(
          id, name, description, league_type, status,
          roster_size, invite_code, season_start, season_end,
          commissioner_id
        )
      `)
  ...
  app.get('/:id', {
    ...
    const { data: league, error } = await supabaseAdmin
      ...
      .select(`
        *,
        commissioner:users!commissioner_id(id, username, avatar_url),
        members:league_members(
          id, team_name, joined_at,
    ...
    // Provjeri pristup za private ligu
    if (league.league_type === 'private') {
      const isMember = league.members.some(m => m.user.id === request.user.id)
      const isCommissioner = league.commissioner_id === request.user.id
  ...
  const { data: testLeagues, error: testError } = await supabaseAdmin
    .from('leagues')
    .select('count')
  ...
  if (league.status !== 'pending') {
    return reply.code(400).send({ error: 'Liga je već počela ili završena' })
  }
  ...
  app.post('/:id/join', {
    ...
    const { data: league, error: leagueError } = await supabaseAdmin
      .from('leagues')
      .select('id, status, max_teams, league_type')
      .eq('id', id)
      .single()
  ...
  // ─── STANDINGS (tabela) ─────────────────────────────────────
  app.get('/:id/standings', {
    onRequest: [authenticate]
  }, async (request, reply) => {
    const { id } = request.params

    // Provjeri da li je član
    ...
    if (error) {
      app.log.error(error)
      return reply.code(500).send({ error: 'Greška pri dohvatanju standings' })
    }
  ...
  app.patch('/:id', {
    ...
    if (!league || league.commissioner_id !== request.user.id) {
      return reply.code(403).send({ error: 'Samo komisar može da menja ligu' })
    }

    const allowed = ['name', 'description', 'status', 'picks_deadline',
                     'season_start', 'season_end', 'scoring_rules', 'tier_limits']
    const updates = {}
  ...
  app.delete('/:id/leave', {
    ...
    // Komisar ne može da napusti sopstvenu ligu
    const { data: league } = await supabaseAdmin
      .from('leagues')
      .select('commissioner_id')
      .eq('id', id)
    ...
    if (league?.commissioner_id === request.user.id) {
      return reply.code(400).send({ error: 'Komisar ne može da napusti ligu — prvo prenesi ulogu ili obrisi ligu' })
    }
