...
export default async function playerRoutes(app) {
  ...
  app.get('/', {
    ...
    const { page = 1, limit = 20, search, tier, title, country, league_id } = request.query
    ...
    if (error) {
      app.log.error(error)
      return reply.code(500).send({ error: 'Greška pri dohvatanju igrača' })
    }
  ...
  app.get('/:id', {
    ...
    if (error || !player) {
      return reply.code(404).send({ error: 'Igrač nije pronađen' })
    }
  ...
  app.get('/roster/:league_id', {
    ...
    if (error) {
      app.log.error(error)
      return reply.code(500).send({ error: 'Greška pri dohvatanju rostera' })
    }
  ...
  app.post('/roster/:league_id', {
    ...
    if (league.status === 'completed') {
      return reply.code(400).send({ error: 'Sezona je završena' })
    }

    if (league.picks_deadline && new Date() > new Date(league.picks_deadline)) {
      return reply.code(400).send({ error: 'Rok za odabir igrača je istekao' })
    }
    ...
    if (error) {
      ...
      if (error.message.includes('Roster je popunjen')) {
        return reply.code(400).send({ error: error.message })
      }
      if (error.message.includes('tier igrača')) {
        return reply.code(400).send({ error: error.message })
      }
      if (error.message.includes('već u nekom timu')) {
        return reply.code(409).send({ error: error.message })
      }
      if (error.code === '23505') {
        return reply.code(409).send({ error: 'Ovaj igrač je već u tvom timu' })
      }

      return reply.code(500).send({ error: 'Greška pri dodavanju igrača' })
    }
  ...
  app.delete('/roster/:league_id/:chess_player_id', {
    onRequest: [authenticate]
  }, async (request, reply) => {
    ...
    if (league.picks_deadline && new Date() > new Date(league.picks_deadline)) {
      return reply.code(400).send({ error: 'Rok za izmene je istekao' })
    }
    ...
    if (error) {
      app.log.error(error)
      return reply.code(500).send({ error: 'Greška pri uklanjanju igrača' })
    }

    return reply.send({ message: 'Igrač uklonjen sa rostera' })
  })
}