...
export default function PickPlayerScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Greška', 'Nije moguće učitati igrače')
    } finally {
      setLoading(false)
    }
  }, [id, search, selectedTier])

  useEffect(() => { load() }, [search, selectedTier])

  const handleAdd = async (playerId: string, playerName: string) => {
    setAdding(playerId)
    try {
      await playerService.addToRoster(id, playerId)
      Alert.alert('Dodato!', `${playerName} je dodat u tvoj tim`, [
        { text: 'Nastavi', style: 'cancel' },
        { text: 'Nazad na tim', onPress: () => router.back() }
      ])
      load()
    } catch (err: any) {
      Alert.alert('Greška', err.response?.data?.error || 'Neuspešno dodavanje')
    } finally {
      setAdding(null)
    }
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Izaberi igrača</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={styles.emptyText}>Nema dostupnih igrača</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  addBtn: {
    backgroundColor: '#22c55e', width: 36, height: 36,
