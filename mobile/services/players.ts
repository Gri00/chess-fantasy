...
export const playerService = {
  ...
  async getAvailable(
    ...
    const { data } = await api.get(`/players/available/${league_id}`, {
      params,
    });
    return data;
  },

  async getRoster(league_id: string, member_id?: string) {
    const params = member_id ? { member_id } : {};
    const { data } = await api.get(`/players/roster/${league_id}`, { params });
    return data.roster;
  },
  ...
  async removeFromRoster(league_id: string, chess_player_id: string) {
    const { data } = await api.delete(
      `/players/roster/${league_id}/${chess_player_id}`,
    );
    return data;
  },
};
